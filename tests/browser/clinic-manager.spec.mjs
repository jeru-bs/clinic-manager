...
test("parents and professionals can be viewed and edited from the patient card", async ({ page }) => {
  ...
  await page.route("https://sheets.googleapis.com/**", async (route) => {
    ...
    if (decoded.includes("fields=sheets.properties")) {
      return route.fulfill({ json: sheetPropertiesJson(Object.keys(headers)) });
    }
    if (decoded.includes("/values:batchGet")) {
      return route.fulfill({
        json: batchGetJson(request, (range) => {
          const [sheet, cells] = range.split("!");
          if (cells === "1:1") return [headers[sheet]];
          if (sheet === "patients") return [patientRow];
          if (sheet === "contacts") return [contactRow];
          if (sheet === "questionnaire_assignments") return appendedRows.questionnaire_assignments || [];
          return [];
        })
      });
    }
